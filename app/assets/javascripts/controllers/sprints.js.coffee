...
app.controller 'SprintsCtrl', @SprintsCtrl = ($scope, $routeParams, Project, Sprint) ->
  ...
  $scope.sprints = Sprint.query project_id: $routeParams.project_id

  $scope.create = (begin_date, end_date) ->
    Sprint.save project_id: $routeParams.project_id, begin_date: begin_date, end_date: end_date, (sprint) ->
      $scope.sprints.push sprint

  $scope.delete = (index) ->
    Sprint.delete project_id: $routeParams.project_id, sprint_id: $scope.sprints[index].id, () ->
